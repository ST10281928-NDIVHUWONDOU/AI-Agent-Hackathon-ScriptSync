...
const TYPE_RULES: Array<[string, RegExp]> = [
  ...
  ['electrical', /electric|power|cable|socket|spark|buzzing|light|contactor/], ['it', /wi-?fi|network|login|authentication|connect|switch|computer|desktop|system/],
...
export function normalizeLocation(value: string | undefined): string { return clean(value).replace(/\blevel\s+(\w+)/g, 'level $1').replace(/\bblock\s+/g, 'block ').trim(); }
...
export function inferType(category: string | undefined, description: string | undefined): string {
  ...
  // A supplied category is weak evidence, but remains more stable than incidental words such as "power" in a water-leak report.
  if (known.has(categoryType)) return categoryType;
  const evidence = `${categoryKey} ${clean(description)}`; const hit = TYPE_RULES.find(([, matcher]) => matcher.test(evidence));
